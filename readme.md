<h1 align="center">Hey, I'm Muhammad Taezeem 👋</h1>
<!-- TxA:29/07/2026 -->
<p align="center">
  <em>Self-taught Dev ⚡ | Ethical Hacker in Training 🕵️ | Passionate Builder 🚀</em>
</p>
<p align="center">
  <a href="https://taezeem.is-a.dev/Clusters/srm"><img src="https://img.shields.io/badge/Grade%208-%40%20SRM%20Welkin-7c5cfc?style=flat-square"/></a>
  <img src="https://img.shields.io/badge/Sopore%2C%20J%26K-🏔️-00e5ff?style=flat-square"/>
  <img src="https://img.shields.io/badge/Open%20to-Collabs-ff4b6e?style=flat-square"/>
</p>

---

### `// About Me`
- 🧠 Grade 8 student going hard on web dev & cybersecurity
- 🛠️ Building with **HTML, CSS, JS, React, Python & Flask**
- 🧅 Explored the **deep & dark web** via sandboxed tools — strictly educational research
- 👓 Ethically pentested my school's site using Burp Suite — **full dept. permission**
- 🚀 Obsessed with building creative, secure & polished projects
- 🕶️ Currently into: AI integrations, PWAs & ethical hacking

---

### `// Tech Stack`
[![My Skills](https://skillicons.dev/icons?i=html,css,azure,androidstudio,react,py,js,flutter,kali,windows,linux,vscode)](https://skillicons.dev)

---

### `// Featured Projects`
| Project | Description | Lang | Stars |
|---|---|---|---|
| 🤖 **[spectrix](https://github.com/taezeem14/spectrix)** | PWA AI chatbot — streaming, voice I/O, math rendering & multi-model routing. 799+ commits. | HTML | ⭐ 5 |
| 🔓 **[BlueCrack](https://github.com/taezeem14/BlueCrack)** | Ethical brute-force tool — built for controlled pentesting environments only. | Python | ⭐ 2 |
| 🐧 **[modded-ubuntu](https://github.com/taezeem14/modded-ubuntu)** | Full Ubuntu GUI on Termux with extended features. *(forked & improved)* | Shell | ⭐ 2 |
| 🕵️ **[DeepRecon](https://github.com/taezeem14/DeepRecon)** | Dark web recon tool — educational & research use only. | Python | ⭐ 1 |
| 📡 **[WirelessADB](https://github.com/taezeem14/WirelessADB)** | Wireless ADB for Android devices below Android 11. No root needed. | Python | ⭐ 6 |
| 🖥️ **[termux-desktop](https://github.com/taezeem14/termux-desktop)** | Full desktop env setup for Termux. Desktop in your pocket, no cap. | Shell | ⭐ 1 |

---

### `// Cyber Exploration`
- Deployed **Tor inside sandboxed environments** (Kasm + VPN + isolated browser)
- Visited **.onion sites** for research via Hidden Wiki
- Practiced anonymity, footprint clearing & network privacy
- Ethically pentested a live target with school's explicit permission using Burp Suite

> 🧠 "Accessed the dark web. Didn't break laws. Just learned more than most textbooks could teach."
> ⚠️ Educational purposes only — not affiliated with or supporting any illegal activity.

---

### `// Roadmap`
- ✅ Vanilla JS PWA architecture & service workers
- ✅ Cloudflare Workers & serverless backends
- 🔄 Advanced React patterns & Next.js
- 🔄 CTF challenges & web exploitation
- 🔜 Systems programming & low-level networking
- 🔜 Custom Kali Linux ISO builds & OPSEC setups

---

### `// GitHub Stats`
<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com?user=taezeem14&theme=radical&hide_border=true&border_radius=10" alt="GitHub Streak"/>
</p>
<p align="center">
  <img src="https://github-readme-stats-sigma-five.vercel.app/api?username=taezeem14&show_icons=true&theme=radical&hide_border=true"/>
</p>

---

### `// Connect`

[![Telegram](https://img.shields.io/badge/Telegram-@Taezeem__14-2CA5E0?style=flat-square&logo=telegram&logoColor=white)](https://t.me/Taezeem_14)<br/>
[![LinkedIn](https://img.shields.io/badge/LinkedIn-muhammad--taezeem-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/muhammad-taezeem-tariq-matta)<br/>
[![ORCID](https://img.shields.io/badge/ORCID-0009--0009--5688--4240-A6CE39?style=flat-square&logo=orcid&logoColor=white)](https://orcid.org/0009-0009-5688-4240)<br/>
[![EMAIL](https://img.shields.io/badge/EMAIL-tariqmtaezeem@gmail.com-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:tariqmtaezeem@gmail.com)

- 🌎 [taezeem.is-a.dev](https://taezeem.is-a.dev)
- ✨ [spectrix.netlify.app](https://spectrix.netlify.app)
- ✨ [spectrix-ai.vercel.app](https://spectrix-ai.vercel.app)

Drop a ⭐ on a repo you like. DMs open for collabs, CTF teams & code reviews.

---

### `// Motto`
> "I don't just browse the web — I build it. Then ethically try to break it." 💻🕵️‍♂️

---

![counter](https://count.getloli.com/@taezeem14?theme=minecraft)

<p align="center">💥 Let's YIBAMBE the web together 🔥</p>
